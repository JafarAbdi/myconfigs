...
If you need to fill out a PDF form, first check to see if the PDF has fillable form fields. Run:
 `uv run {baseDir}/scripts/check_fillable_fields <file.pdf>`, and depending on the result go to either the "Fillable fields" or "Non-fillable fields" and follow those instructions.

# Fillable fields
If the PDF has fillable form fields:
- Run: `uv run {baseDir}/scripts/extract_form_field_info.py <input.pdf> <field_info.json>`. It will create a JSON file with a list of fields in this format:
```
[
  {
...
`uv run {baseDir}/scripts/convert_pdf_to_images.py <file.pdf> <output_directory>`
...
```
- Run the `fill_fillable_fields.py` script to create a filled-in PDF:
`uv run {baseDir}/scripts/fill_fillable_fields.py <input pdf> <field_values.json> <output pdf>`
...
`uv run {baseDir}/scripts/extract_form_structure.py <input.pdf> form_structure.json`
...
`uv run {baseDir}/scripts/check_bounding_boxes.py fields.json`
...
`uv run {baseDir}/scripts/convert_pdf_to_images.py <input.pdf> <images_dir/>`
...
`uv run {baseDir}/scripts/fill_pdf_form_with_annotations.py <input.pdf> fields.json <output.pdf>`

## Step 4: Verify Output

Convert the filled PDF to images and verify text placement:
`uv run {baseDir}/scripts/convert_pdf_to_images.py <output.pdf> <verify_images/>`
