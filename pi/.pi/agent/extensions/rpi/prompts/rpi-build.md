...
Run those verification commands yourself. Delegate `correctness-reviewer` with the generated outline
as requirement context; it resolves its own scope with `git`. Fix blocking findings through
`implementer`, rerun verification, and review again. Stop when no blocking finding remains or a
round changes no files. Never fix non-blocking findings.
