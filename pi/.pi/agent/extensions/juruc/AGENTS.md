...
- R3: Every discovery stage, implementation phase, reviewer, feedback grill, correction plan, and correction implementation uses a fresh durable session with only its declared information diet and tools.
...
- R7: `Approve` requires no saved comments and explicit human action. `Send Feedback` sends all saved comments in file/line order through an ordered nested flow of fresh feedback-grill, correction-plan, and correction implementation sessions; confirmed feedback and the accepted bounded correction plan precede implementation, then JURUC verifies and commits the result, freezes the round, and starts a fresh cumulative review round.
...
- I3: The accepted Plan, confirmed correction feedback, accepted correction plans, recorded checkpoints, commits, and completed review rounds are immutable. Open-round comments follow their explicit edit/delete rules; later corrections append history and never rewrite or remap completed state.
...
- C1: Lifecycle stages are exactly `questions`, `research`, `specification`, `plan`, `implementation`, `review`, and `done`; top-level transitions move forward without blocking, backtracking, renewed research, or renewed top-level planning.
...
- N3: Automatic approval, model-driven correction loops, independent per-phase audits, top-level Plan extension, compatibility migration, or speculative recovery machinery.
